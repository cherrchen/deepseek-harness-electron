...
`@dsh-electron/dsh-electron-*` 命名 Desktop-required 适配器与 infrastructure。此类包属于 `apps/electron/runtime/plugins/`，不应放在本目录。Desktop 始终挂载的 Electron 必需 portable UI 基础设施（例如 `@dsh-electron/dsh-client-ui-details-host` 与 `@dsh-electron/dsh-theme-studio`）是 `apps/electron/runtime/plugins/` 下的 git subtree，不是本 island 的成员。
