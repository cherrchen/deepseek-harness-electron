...
A runtime plugin Client bundle containing TSX renders from the rebuilt `lib/client.js` without a React identifier. Coverage is the built Client artifact, not a headed Electron window. The Plugin Manager tab is the TSX occupant pinned by the focused test.
