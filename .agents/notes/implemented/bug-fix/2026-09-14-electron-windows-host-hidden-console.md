...
[`resolveHostRuntime`](../../../../apps/electron/src/runtime.ts) is the single resolution point for every dsh child the Desktop supervises: the supervised Host in [`startHarness`](../../../../apps/electron/src/main.ts) and the `dsh plugin` commands of [`createPluginCommandRunner`](../../../../apps/electron/src/plugin-install.ts). On Windows it resolves, in order, `DSH_ELECTRON_NODE_BINARY`, the packaged `resources/node/node.exe`, and the prepared `.electron-build/node/win-<arch>/node.exe`; a Windows target with none of them fails at startup with a message naming `pnpm --filter @dsh-electron/dsh-electron prepare:node` rather than falling back to Electron. Other platforms keep `process.execPath` with `ELECTRON_RUN_AS_NODE=1`, and that environment reaches both the generated pnpm shim and the plugin command child, neither of which carries the variable on Windows.
