...
Git 控件需要区分定义 agent 工作上下文的 branch 与 repository 检查及变更操作，同时不能挤占 conversation 或 tool-call details。
...
- 插件不注册 `sidebar.footer.action`，也不依赖 `dsh-client-ui-sidebar`。
...
**独立 Git app surface。** 这种设计会将 branch 选择与 Composer 分离，但所选 branch 属于 agent 工作上下文。

**放置在 sidebar 或 details。** Sidebar footer 会弱化 branch 与 Composer 上下文的关系，而 `details` 属于 tool-call details。
