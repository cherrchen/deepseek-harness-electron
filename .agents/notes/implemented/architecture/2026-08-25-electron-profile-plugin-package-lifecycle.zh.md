...
System 与 distribution-bundled package 不具有 package action。Registry-owned direct profile dependency 支持 explicit update check、遵循 range 的 update、reinstall 与 removal。Git 与本地 `file:` dependency 从 recorded `requestedSpec` refresh。Healthy `link:` dependency 使用 runtime reload；incomplete link 可以 repair。Unknown direct profile dependency 允许 removal，但不允许 source operation。通过 CLI 安装的 profile dependency 遵循相同 package policy。健康的普通 runtime-plugin 无需 `profileManaged` 即可管理；该列表只记录 Desktop 安装（[可管理类别说明](2026-09-13-electron-plugin-manageable-kinds.zh.md)）。
