...
Bundled ecosystem plugins are exact production npm dependencies of `@dsh-electron/dsh-electron`. The current Git plugin dependency is `@dsh-electron/dsh-plugin-git@0.2.0`; this repository contains no copy of its source. `dshElectron.ecosystemPlugins` remains the distribution roster and runtime lifecycle classification, but it does not imply workspace membership.
