...
Electron writes `plugin-state.json` and `plugins.cordis.yml` below `$DSH_HOME/electron/`. `plugin-state.json` stores persisted disabled package names and Desktop-managed profile dependency membership. `plugins.cordis.yml` is the generated desired roster for manageable ecosystem plugins, using package names as stable entry ids and preserving catalog order. [Electron profile plugin installation](2026-08-25-electron-profile-plugin-installation.md) owns the refreshable catalog, state version 2, and package installation path that extends this lifecycle to profile-owned plugins.
