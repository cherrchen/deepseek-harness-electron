...
- `__DSH_BOOT__` 的 served 渲染文本从 `window.__DSH_BOOT__ =` 变为 `globalThis["__DSH_BOOT__"] =`；Electron 的 `extractHostBootstrap` 刮取 Host index HTML 时必须匹配该标记；已核实无已提交快照期望含旧文本，无需重录。
