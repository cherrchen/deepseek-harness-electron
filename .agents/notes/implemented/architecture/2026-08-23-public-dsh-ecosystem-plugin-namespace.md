...
`@dsh-electron/dsh-plugin-*` identifies public DSH ecosystem features. The publisher scope does not imply Electron. Portable plugins use upstream DSH/Cordis services; Desktop-aware plugins keep the main fiber portable and install native behavior in an optional `ctx.inject(['desktop'], ...)` child fiber against the smallest structural interface they consume. `@dsh-electron/dsh-electron-*` remains reserved for Desktop-required adapters and infrastructure under `apps/electron/runtime/plugins/`. Electron-required portable UI infrastructure may also live there as a subtree of a public `platform: web` package; [that category](2026-08-24-electron-required-portable-ui-infrastructure.md) is not an ecosystem plugin.
...
Electron lists standard ecosystem packages in `dshElectron.ecosystemPlugins` and as production `workspace:` dependencies, discovers their installed artifacts under the app `node_modules`, links them into the supervised Host profile, and mounts them through generated composition. Its custom-scheme handler sends known Host read paths and every non-read request to the supervised process, so plugin-owned Connection RPC channels do not collide with Renderer file routing. The Desktop runtime builder compiles every directory under `apps/electron/runtime/plugins/` from source. Public ecosystem plugins under `packages/dsh-electron/` retain independently built artifacts.
