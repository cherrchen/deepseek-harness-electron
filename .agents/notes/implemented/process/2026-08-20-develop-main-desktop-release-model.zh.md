...
- [`desktop-release.yml`](../../../../.github/workflows/desktop-release.yml) 从 tag push 打包安装程序，并验证 Beta tag 属于 `develop`，RC 和 Stable tag 属于 `main`。Windows 包关闭构建器内部的差分 7z 归档并使用会报告解压失败的 NSIS ZIP 载荷路径，同时通过无 scope 的打包应用名称获得稳定的 updater 缓存目录。每个 Windows runner 都会安装已完成的产物，检查应用可执行文件、打包 runtime、桌面快捷方式与开始菜单快捷方式，然后在上传前卸载。
...
**保留默认的内嵌 7z 载荷。** 构建器的 7z NSIS 宏可能在临时解压结果为空后继续运行，并仍然写入卸载程序和快捷方式。ZIP 载荷更大，但其 NSIS 解压路径会检查并报告解压失败。若启用 ZIP 解压却不关闭内部差分包，构建器仍会生成 7z 字节，却指示 NSIS 将其作为 ZIP 打开，因此该组合无效。

## 后果

上游集成和 Beta release 在 `develop` 上进行；最终 Beta commit 只推送一次，由该 push 触发的 Desktop CI run 验证，并仅在该 run 成功后创建 tag。RC 和 Stable release 要求通过 Squash Merge 升级到 `main`。发布 PR 携带 Electron 版本和 lockfile 更新，合并后的 workflow 只写入 release tag。Windows 安装程序以一部分压缩率换取可检查的解压，并增加原生安装/卸载冒烟测试的发布时间。README 入口页仍由下游维护；`AGENTS.md` 跟随上游，并恢复下游引用。旧版 `electron-dsh-v*` tag 保留为历史产物；新 release 使用 `v{a.b.c}[-beta.x|-rc.x]`。
