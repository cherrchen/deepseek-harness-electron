...
- Portable 与 Desktop-aware 产品功能归属 `packages/dsh-electron/` 下的标准 DSH/Cordis package；`runtime/plugins/` 仅保留 Desktop-required adapter 与 carrier integration。Desktop-required Host 组合留在 `runtime/host.patch.yml`；bundled 生态插件成员资格由生成的 `$DSH_HOME/electron/plugins.cordis.yml` roster 决定（[插件生命周期](../../docs/electron/plugin-lifecycle.zh.md)）。
