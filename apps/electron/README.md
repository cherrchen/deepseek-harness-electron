...
The release resources include the Rust Network Runtime at a fixed application-owned path. Manual and System start and configure its Gateway before Harness; Direct applies explicit Electron routing and clears controlled child proxy variables. Default leaves Electron proxy settings and inherited child environments untouched. The updater, plugin commands, and Agent subprocess provider follow their distinct Desktop network policies; [Network Runtime](../../docs/electron/network-runtime.md) records the supported protocols and current limits.
