...
- Portable and Desktop-aware product features belong in independently published DSH/Cordis packages; `runtime/plugins/` holds Desktop-required adapters and Electron carriers; Electron-required portable DSH UI infrastructure ships as a published package declared in `dshElectron.runtimePlugins`. Desktop-required Host composition and bundled Git are listed in `runtime/host.patch.yml` ([plugin lifecycle](../../docs/electron/plugin-lifecycle.md)).
...
OS desktop capabilities are owned by Electron Main and exposed through the typed `window.deepseekDesktop` preload bridge. The Desktop Capability Provider plugin (`runtime/plugins/desktop-capabilities`) adapts that bridge into `ctx.desktop` for feature plugins. The supervised Host receives `runtime/host.patch.yml`, which mounts the required Desktop adapters and bundled Git plugin. Theme Studio is bundled and linked but disabled until its published dsh peer range supports this release. At startup, Main validates bundled artifacts and links them into `$DSH_HOME/profiles/node_modules`. It transfers previously Desktop-managed runtime plugins into the Web profile patch once, preserving their enabled or disabled state. The upstream Web profile then owns plugin management and its UI; Main provides the packaged pnpm executable on the Host `PATH`. `scripts/build-runtime-plugins.mjs` builds local Desktop plugins from source, while Theme Studio and Git retain their published Host and Client artifacts. Upstream UI clipboard writes use a renderer shim to reach Main. External URLs opened as new windows use `https:`, `http:`, or `mailto:`.

The bundled Desktop network subprocess plugin imports the installed `dsh-subprocess-local` and `dsh-subprocess` packages at runtime. They must share the Host's executable-not-found error class so terminal shell discovery can skip unavailable candidates.
...
A Host startup timeout stops the supervised child before Desktop reports the failure. The upstream profile manager owns its package transactions and profile files.
