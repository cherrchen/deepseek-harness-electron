...
The main window uses a hidden title bar without a separate heading row. The sidebar and conversation backgrounds extend to the window top: the sidebar's native-control inset and unused parts of the active conversation header are draggable, while a transparent 40-pixel hit surface covers the empty-session background. Header controls and full-viewport dialogs are explicitly non-draggable. macOS positions its traffic lights over the sidebar; Windows and Linux use Electron's Window Controls Overlay for native minimize, maximize, and close controls. Closing the main window hides it while the Harness process continues running. Use the tray menu to reopen the window or quit the application and stop the supervised process.
