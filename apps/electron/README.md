...
OS desktop capabilities are owned by Electron Main and exposed through the typed `window.deepseekDesktop` preload bridge. The Desktop Capability Provider plugin (`runtime/plugins/desktop-capabilities`) adapts that bridge into `ctx.desktop` for feature plugins. Main owns plugin lifecycle through `ctx.desktop.plugins`; the Installed settings tab and its reads, mutations, polling, rollback, and Renderer refresh behavior are documented in the [plugin lifecycle reference](../../docs/electron/plugin-lifecycle.md) and are not a bootstrap mount in this pin. The supervised Host receives an `apps/electron/runtime` cordis overlay that disables Host `directory-picker-auto`, keeps the browse Host backend so `directoryPicker` still injects for apiproxy, mounts the capability provider, Theme Studio, an Electron-local directory-flow client plugin (no browse client), and an Electron-local brand plugin that always fills the shipped brand slots, and installs a `cordis:include` seat whose generated `$DSH_HOME/electron/plugins.cordis.yml` roster is empty until an ecosystem plugin is declared. `scripts/build-runtime-plugins.mjs` rebuilds every `runtime/plugins/` directory from source; standard ecosystem plugins retain their independently built Host and Client artifacts. Both categories are linked into `$DSH_HOME/profiles/node_modules` and `$DSH_HOME/electron/node_modules` at startup. Upstream UI clipboard writes are redirected through a narrow renderer shim to Main until an upstream injection seam exists. External URLs opened as new windows must use `https:`, `http:`, or `mailto:`.

Plugin package commands retain their transaction until the child closes. Spawn errors reject the command; a failed lock-owner handoff terminates the child and rejects after close.
...
On macOS, startup reports Keychain storage as supported without accessing it. Saving a Manual proxy password or loading one configured for the active Manual route checks storage availability and may request Keychain access. A missing stored password does not access Keychain.

The release resources include the Rust Network Runtime at a fixed application-owned path. Manual and System start and configure its Gateway before Harness; Direct applies explicit Electron routing and clears controlled child proxy variables. Default leaves Electron proxy settings and inherited child environments untouched. With Agent proxying disabled, the Agent subprocess provider preserves inherited and Harness-home `.env` proxy values. The updater, plugin commands, and Agent subprocess provider follow their distinct Desktop network policies; [Network Runtime](../../docs/electron/network-runtime.md) records the supported protocols and current limits.
...
Startup recovery and workspace-policy writes acquire the web-profile lock before changing files. A Host timeout permits recovery only after shutdown succeeds; a shutdown failure stops startup. Recovery excludes catalogued unloadable bundles from the profile stack without deleting their dependency entries. Reserved-name rollback children own the lock until close. After repair, composition and lifecycle operations share freshly loaded plugin preferences. The workspace policy accepts block and inline YAML mappings, retaining user build overrides and unrelated values.
