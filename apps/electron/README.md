...
The main window uses a hidden title bar with a 40-pixel drag strip above the Electron renderer. That strip is the renderer's only drag region; session content and full-viewport overlays remain interactive. macOS keeps its traffic lights in the strip, while Windows and Linux use Electron's Window Controls Overlay for native minimize, maximize, and close controls. Closing the main window hides it while the Harness process continues running. Use the tray menu to reopen the window or quit the application and stop the supervised process.
