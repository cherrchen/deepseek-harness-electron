...
Packaged builds default to the **Pre-Release** channel and persist the selected channel in Electron's user-data directory. The application and tray menus can switch between **Pre-Release**, which receives the newest published prerelease or stable release, and **Stable / Release**, which uses GitHub's latest production release and never selects a prerelease. Channel selection uses GitHub Release metadata rather than interpreting the repository's `electron-dsh-v<version>` tags; `electron-updater` still owns metadata validation, semantic-version comparison, download, and installation.
