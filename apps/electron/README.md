...
The main window uses a hidden title bar without a separate heading row. The sidebar background extends to the window top: macOS reserves a draggable sidebar inset for its traffic lights, and Windows and Linux start the columns right of the sidebar below the native control row, leaving that band blank and draggable. Unused parts of the active conversation header are draggable, and a transparent hit surface covers the empty-session background and the blank band. Header controls are explicitly non-draggable, and every page drag region is suspended while a modal dialog is open so its mask and controls retain pointer input. Closing the main window hides it while the Harness process continues running. Use the tray menu to reopen the window or quit the application and stop the supervised process.
