...
Startup recovery and workspace-policy writes acquire the web-profile lock before changing files. A Host timeout permits recovery only after shutdown succeeds; a shutdown failure stops startup. After repair, composition and lifecycle operations share freshly loaded plugin preferences. The workspace policy accepts block and inline YAML mappings, retaining user build overrides and unrelated values.
