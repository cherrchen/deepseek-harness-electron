...
The Desktop Network Runtime routes HTTP and CONNECT traffic through one Manual HTTP, HTTPS, or unauthenticated SOCKS5 proxy. It never substitutes Direct when that proxy fails. The Runtime and Main client are independently usable infrastructure; Desktop startup, Electron sessions, updater, plugin commands, and Settings UI do not yet activate them.
...
The supported commands are `hello`, `configure`, `get_diagnostics`, and `shutdown`. `configure` accepts `{ config, limits? }`, using the [RuntimeNetworkConfig](../../apps/electron/src/network/domain.ts) union. Direct is available for connector tests and explicit direct routing. System configuration returns `SYSTEM_PROXY_BACKEND_UNAVAILABLE`; advertised System and integrated-auth capabilities are false. Unsupported commands and invalid payloads return an error response; malformed envelopes emit `runtime_warning`. Neither changes the active configuration.
